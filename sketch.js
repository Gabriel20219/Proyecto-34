...
function setup() {
  ...
  zombie1 = new Zombie(width - 300, 330, 50, 200);
  zombie2 = new Zombie(width - 550, height - 300, 50, 200);
}
...
function draw() {
  ...
  for (var i = 0; i < playerArrows.length; i++) {
    if (playerArrows[i] !== undefined) {
      ...
      var zombie1Collision = Matter.SAT.collides(
        zombie1.body,
        playerArrows[i].body
      );

      var zombie2Collision = Matter.SAT.collides(
        zombie2.body,
        playerArrows[i].body
      );

      if (zombie1Collision.collided || zombie2Collision.collided) {
        score += 5;
      }
